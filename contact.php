<html>
<head>
<title>BS FORUM</title>
        <link rel="stylesheet" href="style.css" type="text/css" />
        <style type="text/css">
<!--
#Layer1 {
	position:absolute;
	left:689px;
	top:175px;
	width:253px;
	height:738px;
	z-index:1;
}
.style1 {color: #FFFFFF}
.style2 {font-size: 16px}
.style3 {font-size: 14px}
-->
        </style>
</head>
    <body>
        <div id="Layer1">
          <div class="sidebar">
            <div id="search"></div>
            <div class="sidebarads">
              <div align="center"><img src="images/AZNH.JPG" width="125" height="150" border="4">              </div>
              <div id="div"></div>
            </div>
            <ul>
              <li class="pagenav">
                <h2>Menu</h2>
                <ul>
                  <li class="page_item page-item-2"><a href="index.php" title="BS FORUM">Home </a></li>
                  <li class="page_item page-item-10"><a href="contact.php?page_id=10" title="Contact">Contact</a></li>
                  <li class="page_item page-item-8"><a href="signup.php?page_id=8" title="Forum">Register</a></li>
                  <li class="page_item page-item-9"><a href="http://www.free-css-templates.com/wordpress/?page_id=9" title="Free Stuff">Gallary</a></li>
                  <li class="page_item page-item-7"><a href="result.php?page_id=7" title="Links">Results</a></li>
                </ul>
              </li>
              <li>
                <h2>Login Here </h2>
                <ul>
                  <li class="cat-item cat-item-3">
				  <form name="form1" method="post" action="login-exec.php">
                    <table width="241" height="100" border="0">
                      <tr>
                        <td width="73"><span class="style1">Login Id </span></td>
                        <td width="158">
                          <label>
                            <input name="login" type="text" id="login">
                          </label>                        </td>
                      </tr>
                      <tr>
                        <td><span class="style1">Password</span></td>
                        <td><label>
                            <input name="password" type="password" id="password">
                          </label>                        </td>
                      </tr>
                      <tr>
                        <td align="right">
						<input name="Login" type="submit" value="Login" align="right">						</td>
						<td><input type="reset" name="Cancel"></td>
					  </tr>
                    </table>
					</form>
                  </li>
                </ul>
              </li>
    <li>
                <h2>News</h2>
                <table width="150" height="200">
				<tr><marquee direction="up" scrolldelay="150">
				<ol class="Layer1">
                  <li>This site is Under Contruction</li>
                	<li>All the BSCS Students get the results and information about his previous progress that would be see here.</li>
					<li>I hope all the students get most benifit from this site</li>
				<li>Plz Pray for me</li>
				</ol>
				</marquee></tr>
				</table>
              </li>
            </div>
        </div>
        <div id="wrapper">
            <div id="header">
            	<div id="header_left">
	                <h1>
	                	
	                        BS	Forum                   
					</h1>
					<h2>The Islamia Univeristy of Bahawalpur<br />(Bahawalnagar Campus) </h2>
			  </div>
				<div id="header_right">
					<div class="rss">					  <img src="images/rss.gif" alt="RSS Feeds" width="25" height="15" align="absmiddle">&nbsp;&nbsp;RSS Posts </div>
					<div class="rss">
						<a href="#"><img src="images/rss.gif" alt="RSS Feeds" width="25" height="15" align="absmiddle">&nbsp;&nbsp;RSS Comments</a>					</div>
				</div>
            </div>
	<div id="content">
		<div id="container">
						<div class="post">
				<h2 class="title">Admin's of this Forum </h2>
				<div class="entry">
					<p>
					<h3 class="h1 style2">Contact Us</h3>
					<hr  />
<p class="entry style3">In this forum you are all the members will contacts the admin's of this fourm. The admin's of this forum are...</p>
<p class="entry style3">&nbsp;</p>
<table width="97%" border="0">
  <tr>
    <td width="51%"><span class="entry">Ali Zulqernain BS(CS) 5th samester <br />
Session 2008-12. <br />
Email id: aznh786@yahoo.com<br />
Mobile No. 0333-6320730</span><hr /></td>
    <td width="49%"><span class="entry">Ammar YasirBS(CS) 5th samester <br />
Session 2008-12. <br />
Email id: ammaryasier@yahoo.com<br />
Mobile No. </span><hr /></td>
  </tr>
  <tr>
    <td><span class="entry">Azhar Hussain BS(CS) 5th samester <br />
Session 2008-12. <br />
Email id: azhr17@yahoo.com<br />
Mobile No. 0333-6301160</span><hr /></td>
    <td>&nbsp;</td>
  </tr>
</table></p>
                    </div>
					<div id="clear"></div>
				</div>
				<div class="postmetadata"> 
					<div>
						<div class="data_category">From&nbsp;</div>
						<div class="data_author"> : Admin</div>
						<div id="clear"></div>
					</div>
					<div id="clear"></div>
				</div>
	  </div>
						<div class="navigation">
					<div class="left-nav"></div>
					<div class="right-nav"> </div>
				</div>
							<div id="clear">
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							  <p>&nbsp;</p>
							</div>
		</div>
		<div id="clear"></div>
	</div>
<div id="footer">		
	<div class="designby">
		(C) Copy Rights Reserved From 2010 ltd.
	</div>
</div></div>

</body></html>